...
import { matchesKey, Key, truncateToWidth, visibleWidth, type AutocompleteItem } from "@earendil-works/pi-tui";
...
function getAllInstalledSkills(cwd: string, rawSystemPromptSkills?: any[]): SkillItem[] {
	...
	try {
		...
		const extraPaths = [userSkillsDir, userAgentsDir, projectSkillsDir, projectAgentsDir].filter((p) =>
			fs.existsSync(p)
		);
...
class SkillsSelectorComponent {
	...
	public onSelect?: (result: { action: "insert"; skillName: string } | { action: "cancel" }) => void;
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, Key.space) || data === " ") {
			if (this.filteredItems.length > 0) {
				...
				if (item) {
					const scopedDisabledSkills = readDisabledSkills(getDisabledSkillsPath(this.cwd, item.scope));
		...
		if (matchesKey(data, Key.backspace) || matchesKey(data, Key.delete) || data === "\x7f" || data === "\b") {
			if (this.searchQuery.length > 0) {
	...
	render(width: number): string[] {
		...
		const headerBar = `─ ${titleStr} ` + "─".repeat(Math.max(0, width - visibleWidth(titleStr) - 3));
		...
		if (this.filteredItems.length === 0) {
			...
			for (let i = this.scrollOffset; i < endIndex; i++) {
				...
				const statusText = item.disabled
					? t.fg("muted", "[ ]")
					: t.fg("success", "[✓]");

				// Scope tag (using [proj] for project-scoped skills)
				const scopeTag = item.scope === "project"
					? t.fg("accent", "[proj]")
                    : t.fg("dim", "[user]");
				...
				const highlightedDesc = highlightMatch(item.description, this.searchQuery, t, (s) => t.fg("dim", s), matchColorFn);
			...
			if (this.filteredItems.length > this.maxVisible) {
				const scrollInfo = t.fg("dim", `  (${this.selectedIndex + 1}/${this.filteredItems.length})`);
		...
		if (this.filteredItems.length > 0) {
			...
			if (selectedItem) {
				const scopeLabel = selectedItem.scope === "project"
				...
				lines.push(truncateToWidth(fieldNameSelected + t.fg("text", t.bold(selectedItem.name)), width));
				...
				for (const dLine of wrappedDescLines) {
					const matchColorFn = (s: string) => t.fg("warning", t.bold(s));
					const highlightedDLine = highlightMatch(dLine, this.searchQuery, t, (s) => t.fg("text", s), matchColorFn);
	...
	pi.registerCommand("skills", {
		...
		handler: async (args, ctx: ExtensionCommandContext) => {
			...
			const systemPromptOpts =
				typeof ctx.getSystemPromptOptions === "function" ? ctx.getSystemPromptOptions() : ({} as any);
			...
			if (!ctx.hasUI) {
				...
				if (projectSkills.length > 0) {
					...
					for (const s of projectSkills) {
						...
						lines.push(`  - ${s.name} ${status} (Disable Model Invocation: ${s.disableModelInvocation}): ${s.description} (${s.filePath})`);
					}
				}
			...
			const result = await ctx.ui.custom<{ action: "insert"; skillName: string } | { action: "cancel" }>(
				(tui, theme, _keybindings, done) => {
					const selector = new SkillsSelectorComponent(targetSkillItems, theme, ctx.cwd, initialSearchQuery);
